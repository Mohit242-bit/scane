    ...
    try {
      ...
      if (!profileResponse.ok) {
        if (profileResponse.status === 404) {
          // center not onboarded yet, redirect to onboarding
          router.push("/centers/onboarding");
          return;
        }
        throw new Error("Failed to load center profile");
      }

      const centerData = await profileResponse.json();
      setcenter(centerData);

      // Get centers using API
      const centersResponse = await fetch("/api/centers/centers", {
        headers: {
          "Authorization": `Bearer ${(await supabase.auth.getSession()).data.session?.access_token}`
        }
      });
      ...
      <div className="min-h-screen bg-gray-50 flex items-center justify-center">
        <Card className="w-full max-w-md">
          <CardContent className="text-center p-6">
            ...
            <h2 className="text-xl font-semibold mb-2">center Profile Not Found</h2>
            <p className="text-[#5B6B7A] mb-4">
              Your center profile could not be found. Let's set it up!
            </p>
            <Button 
              onClick={() => router.push("/centers/onboarding")}
      ...
      <div className="bg-white shadow-sm border-b">
        <div className="container max-w-6xl mx-auto px-4 py-4">
          <div className="flex items-center justify-between">
            <div>
              <h1 className="text-2xl font-bold text-[#0B1B2B]">{center.business_name}</h1>
              <p className="text-[#5B6B7A]">center Dashboard</p>
            </div>
            <div className="flex items-center gap-3">
              <Badge variant={center.status === "approved" ? "default" : "secondary"}>
                {center.status === "approved" ? "Approved" : "Pending Approval"}
              </Badge>
        ...
        <Tabs defaultValue="centers" className="space-y-6">
          ...
          <TabsContent value="centers" className="space-y-6">
            <div className="flex items-center justify-between">
              ...
              <Button 
                className="bg-[#0AA1A7] hover:bg-[#089098]"
                onClick={() => router.push("/centers/centers/new")}
              >
                      ...
                      <div className="space-y-3">
                        ...
                        <div className="flex gap-2 pt-2">
                          <Button 
                            ...
                            size="sm" 
                            className="flex-1"
                            onClick={() => router.push(`/centers/centers/${center.id}`)}
                          >
                          ...
                          <Button 
                            ...
                            size="sm" 
                            className="flex-1"
                            onClick={() => router.push(`/centers/centers/${center.id}/edit`)}
                          >
              ...
              <CardContent className="space-y-4">
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <p className="text-base font-semibold">{center.business_name}</p>
                  </div>
                  <div>
                  ...
                  <div>
                    ...
                    <p className="text-base">{center.business_email}</p>
                  </div>
                  <div>
                    <label className="text-sm font-medium text-[#5B6B7A]">Business Phone</label>
                    <p className="text-base">{center.business_phone}</p>
                  </div>
                  <div className="col-span-2">
                    <label className="text-sm font-medium text-[#5B6B7A]">Address</label>
                    <p className="text-base">{center.business_address}, {center.city}</p>
                  </div>
                </div>
                <div className="pt-4">
                  <Button variant="outline" onClick={() => router.push("/centers/profile/edit")}>
